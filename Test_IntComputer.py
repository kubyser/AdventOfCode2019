...
from IntComputer import IntComputer, IntComputerThread, Chain
...
class IntComputerTestCase(unittest.TestCase):
    ...
    def test_single_computer(self):
        ...
        for i in range(len(res)):
            self.assertEqual(res[i], comp.readMem(i))
    ...
    def test_day2(self):
        ...
        comp = IntComputer(p)
        comp.compute()
        self.assertEqual(comp.readMem(0), 3516593)
        ...
        comp = IntComputer(p)
        comp.compute()
        self.assertEqual(comp.readMem(0), 19690720)
    ...
    def test_day5(self):
        p = IntComputer.readProgram("day5_input.txt")
    ...
    def test_relativeBase(self):
        p = [20101,22,1,10,109,5,4,10,99]
        self.assertEqual([44], self.runSingleComputer(p, []))
        res = self.runSingleComputer([109,1,204,-1,1001,100,1,100,1008,100,16,101,1006,101,0,99], [])
        self.assertEqual([109,1,204,-1,1001,100,1,100,1008,100,16,101,1006,101,0,99],
                         res)
        self.assertEqual([1219070632396864], self.runSingleComputer([1102,34915192,34915192,7,4,7,99,0], []))
        self.assertEqual([1125899906842624], self.runSingleComputer([104,1125899906842624,99], []))

    def test_day9(self):
        p = IntComputer.readProgram("day9_input.txt")
        self.assertEqual([3340912345], self.runSingleComputer(p, [1]))
