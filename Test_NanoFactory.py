...
class NanoFactoryTestCase(unittest.TestCase):
    def test_fuelproduction(self):
        p1 = ["10 ORE => 10 A","1 ORE => 1 B","7 A, 1 B => 1 C","7 A, 1 C => 1 D","7 A, 1 D => 1 E","7 A, 1 E => 1 FUEL"]
        factory = NanoFactory(p1)
        factory.requestMaterial("FUEL", 1)
        res = factory.getRequiredAmountOf("ORE")
        self.assertEqual(31, res)
        p2 = ["9 ORE => 2 A","8 ORE => 3 B","7 ORE => 5 C","3 A, 4 B => 1 AB","5 B, 7 C => 1 BC","4 C, 1 A => 1 CA",
              "2 AB, 3 BC, 4 CA => 1 FUEL"]
        factory = NanoFactory(p2)
        factory.requestMaterial("FUEL", 1)
        res = factory.getRequiredAmountOf("ORE")
        self.assertEqual(165, res)
        p3 = ["157 ORE => 5 NZVS","165 ORE => 6 DCFZ","44 XJWVT, 5 KHKGT, 1 QDVJ, 29 NZVS, 9 GPVTF, 48 HKGWZ => 1 FUEL",
        ...
        factory = NanoFactory(p3)
        factory.requestMaterial("FUEL", 1)
        res = factory.getRequiredAmountOf("ORE")
        self.assertEqual(13312, res)
        p4 = ["2 VPVL, 7 FWMGM, 2 CXFTF, 11 MNCFX => 1 STKFG","17 NVRVD, 3 JNWZP => 8 VPVL",
        ...
        factory = NanoFactory(p4)
        factory.requestMaterial("FUEL", 1)
        res = factory.getRequiredAmountOf("ORE")
        self.assertEqual(180697, res)
        p5 = ["171 ORE => 8 CNZTR","7 ZLQW, 3 BMBT, 9 XCVML, 26 XMNCP, 1 WPTQ, 2 MZWV, 1 RJRHP => 4 PLWSL",
        ...
        factory = NanoFactory(p5)
        factory.requestMaterial("FUEL", 1)
        res = factory.getRequiredAmountOf("ORE")
        self.assertEqual(2210736, res)
